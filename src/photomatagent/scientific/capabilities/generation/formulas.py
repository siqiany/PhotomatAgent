...
The torch decoding step is dependency-optional: a ``decoder`` callable
(condition -> composition fractions matrix) is injected by the caller. The
default decoder requires a torch checkpoint; when torch or the checkpoint is
missing, tools return a typed ``missing_prerequisites`` failure instead of
guessing. All downstream logic (integer stoichiometry, oxidation-state
plausibility, charge neutrality, novelty) is pure Python and fully tested.
"""
...
class VAEFormulaGenerator:
    ...
    def _load_torch_decoder(self) -> Decoder:
        """Load the donor CVAE checkpoint; raises typed failures."""
        ...
        raise MissingScientificPrerequisite(
            "torch checkpoint decoding requires the donor conditional_vae "
            "model module; not bundled in this environment",
            missing=["conditional_vae module", "vae_checkpoint_path"],
        )
    ...
    def condition(self, *, target_band_gap_eV: float) -> np.ndarray:
        ...
        for index, field in enumerate(self.property_fields):
            if field == "gap_selected_eV":
                condition[index] = (
                    float(target_band_gap_eV) - self.center[index]
                ) / max(self.scale[index], 1e-12)
        return condition
    ...
    def generate(
        ...
        *,
        ...
        if target_band_gap_eV is not None:
            ...
            band_gap = HC_EV_UM / float(target_wavelength_um)
        if band_gap <= 0:
            raise ValueError("target band gap must be positive")
        condition = self.condition(target_band_gap_eV=band_gap)
