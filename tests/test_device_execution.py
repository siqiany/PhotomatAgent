...
async def test_device_script_can_import_devsim(tmp_path, monkeypatch):
    ...
    result = await DeviceRunScriptTool(
        ScientificConfig(), Workspace(tmp_path)
    ).execute({"path": script.name, "timeout_seconds": 10})
...
async def test_device_timeout_terminates_worker(tmp_path, monkeypatch):
    ...
    result = await DeviceRunScriptTool(
        ScientificConfig(), Workspace(tmp_path)
    ).execute({"path": script.name, "timeout_seconds": 1})
